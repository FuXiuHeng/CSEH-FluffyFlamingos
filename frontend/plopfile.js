		...
		},
		{
			...
			validate: (input, answers) => {
				...
				if (sh.test('-e', path.join(input, answers.name)))
					return `'${chalk.yellow(answers.name)}' already exists in ${chalk.yellow(
						parentDirShort,
					)}`;
				if (answers.name.toLowerCase() === path.basename(input).toLowerCase())
					return `Do not include the component's name ('${chalk.yellow(
						answers.name,
					)}') in the parent directory path -- that directory will be created for you`;
...
module.exports = function(plop) {
	...
	plop.setHelper('pascalToHeading', text =>
		text.replace(/([a-z])([A-Z])/g, '$1 $2').toUpperCase(),
	);
	...
	plop.setGenerator(
		...
		makeComponentGenerator(true, 'React (stateless) functional component'),
	);
};
